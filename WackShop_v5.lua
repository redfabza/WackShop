...
-- key แยกได้ เช่น TweenK(obj,"_rot",{Rotation=180},0.28) จะไม่ cancel tween สี
local _tweens = {}
local function TweenK(obj, key, props, t)
    local k = tostring(obj)..key
    if _tweens[k] then pcall(function() _tweens[k]:Cancel() end) end
    local tw = TweenService:Create(obj, TweenInfo.new(t or 0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), props)
    _tweens[k] = tw
    tw:Play()
end
local function Tween(obj, props, t)
    TweenK(obj, "_default", props, t)
end
...
function Library:NewWindow(config)
    ...
    Body.Size=UDim2.new(1,0,1,-46); Body.Position=UDim2.new(0,0,0,46)  -- [FIX] ตรงกับ TBar height 46px
    ...
    Sidebar.BorderSizePixel=0; Sidebar.ZIndex=4  -- [FIX] สูงกว่า ContentArea (ZIndex 2)
    ...
    SideList.ScrollBarThickness=0          -- ซ่อน scrollbar ให้สวย
    ...
    SideList.ElasticBehavior=Enum.ElasticBehavior.Always  -- bounce effect
    SideList.ScrollingEnabled=true
    SideList.VerticalScrollBarInset=Enum.ScrollBarInset.ScrollBar
    ...
    local sideAnimBusy  = false

    -- [FIX] child ของ SG (ScreenGui) แทน Main/Body
    -- เพราะ Main.ClipsDescendants=true ทำให้ปุ่มที่โผล่นอกขอบถูกตัด
    -- ใช้ RenderStepped คอย sync ตำแหน่งตาม Main.AbsolutePosition
    local CollapseBtn = Instance.new("TextButton", SG)
    CollapseBtn.Size     = UDim2.new(0,20,0,48)   -- กว้างขึ้น กดง่ายบน mobile
    CollapseBtn.Position = UDim2.new(0,0,0,0)  -- sync ด้วย RenderStepped
    CollapseBtn.BackgroundColor3 = Theme.Surface2
    CollapseBtn.Text     = "‹"
    CollapseBtn.TextColor3 = Theme.Accent
    CollapseBtn.Font     = Enum.Font.GothamBold
    CollapseBtn.TextSize = 16
    CollapseBtn.ZIndex   = 20
    CollapseBtn.AutoButtonColor = false
    CollapseBtn.Selectable = false
    Corner(CollapseBtn, 6)
    Stroke(CollapseBtn, Theme.Border, 1)

    -- offset X: ปุ่มกว้าง 20 ครึ่งนึง(10) โผล่นอก sidebar
    -- collapsed=false → SideW-10, collapsed=true → -10
    local collapseBtnOffsetX = SideW - 10

    local function SyncCollapseBtn()
        local mp = Main.AbsolutePosition
        local ms = Main.AbsoluteSize
        -- guard: ถ้า AbsoluteSize ยัง 0 (layout ยังไม่เสร็จ) ข้ามไปก่อน
        if ms.X == 0 then return end
        CollapseBtn.Position = UDim2.new(
            0, mp.X + collapseBtnOffsetX,
            0, mp.Y + 46 + (ms.Y - 46)/2 - 24
        )
    end
    -- sync ครั้งแรกหลัง layout เสร็จ
    task.defer(SyncCollapseBtn)

    -- hover glow — ใช้ key "_bg" แยกจาก "_rot" ป้องกัน tween ชนกัน
    CollapseBtn.MouseEnter:Connect(function()
        TweenK(CollapseBtn,"_bg",{BackgroundColor3=Color3.fromRGB(35,35,55)},0.1)
        TweenK(CollapseBtn,"_txt",{TextColor3=Color3.fromRGB(255,255,255)},0.1)
    end)
    CollapseBtn.MouseLeave:Connect(function()
        TweenK(CollapseBtn,"_bg",{BackgroundColor3=Theme.Surface2},0.1)
        TweenK(CollapseBtn,"_txt",{TextColor3=Theme.Accent},0.1)
    end)

    local ANIM_T = 0.28

    local function CollapseToggle()
        if sideAnimBusy then return end
        sideAnimBusy = true
        sideCollapsed = not sideCollapsed

        if sideCollapsed then
            -- ── พับเข้า ──
            for _,v in ipairs(SideList:GetDescendants()) do
                if v:IsA("TextLabel") then Tween(v,{TextTransparency=1}, ANIM_T*0.5) end
            end
            Tween(BrandIcon,{TextTransparency=1}, ANIM_T*0.5)

            task.delay(ANIM_T*0.35, function()
                Tween(Sidebar,{Size=UDim2.new(0,0,1,0)}, ANIM_T)
                Tween(ContentArea,{
                    Size    = UDim2.new(1,-1,1,0),
                    Position= UDim2.new(0,1,0,0)
                }, ANIM_T)
                -- เลื่อนปุ่มโดยค่อยๆ เปลี่ยน collapseBtnOffsetX ผ่าน tween proxy
                -- (ปุ่มอยู่ใน SG ไม่มี parent frame → tween UDim2 Position ตรงๆ ได้)
                TweenK(CollapseBtn,"_rot",{Rotation=180}, ANIM_T)
                -- lerp offset จาก SideW-8 → -8 แบบ manual ใน task.spawn
                local startOff = collapseBtnOffsetX
                local endOff   = -10
                local elapsed  = 0
                task.spawn(function()
                    while elapsed < ANIM_T do
                        local dt = task.wait()
                        elapsed = math.min(elapsed + dt, ANIM_T)
                        local t = elapsed / ANIM_T
                        -- sine ease out
                        local e = math.sin(t * math.pi / 2)
                        collapseBtnOffsetX = startOff + (endOff - startOff) * e
                        SyncCollapseBtn()
                    end
                    collapseBtnOffsetX = endOff
                    SyncCollapseBtn()
                end)
            end)

            task.delay(ANIM_T*0.35 + ANIM_T + 0.05, function()
                sideAnimBusy = false
            end)
        else
            -- ── เปิดกลับ ──
            ...
            Tween(Sidebar,{Size=UDim2.new(0,SideW,1,0)}, ANIM_T)
            TweenK(CollapseBtn,"_rot",{Rotation=0}, ANIM_T)
            local startOff = collapseBtnOffsetX
            local endOff   = SideW - 10
            local elapsed  = 0
            task.spawn(function()
                while elapsed < ANIM_T do
                    local dt = task.wait()
            ...
            task.delay(ANIM_T*0.6, function()
                for _,v in ipairs(SideList:GetDescendants()) do
                    if v:IsA("TextLabel") then Tween(v,{TextTransparency=0}, ANIM_T*0.5) end
                end
                Tween(BrandIcon,{TextTransparency=0}, ANIM_T*0.5)
            end)

            task.delay(ANIM_T + 0.05, function()
                sideAnimBusy = false
            end)
        end
    end
    ...
    CollapseBtn.InputEnded:Connect(function(i)
    ...
    -- [FIX 3] SetVisible — toggle Main + CollapseBtn ด้วยกันเสมอ
    local visible = true
    ...
    local function SetVisible(v)
        ...
        if v then
            ...
            CollapseBtn.Visible = true   -- [FIX] แสดงปุ่มพร้อม Main
            ...
            task.delay(0.2, function()
                ...
                CollapseBtn.Visible = false  -- [FIX] ซ่อนปุ่มพร้อม Main
                animBusy = false
            end)
        end
    end
    -- [FIX] รับ Touch ด้วย InputEnded แทน MouseButton1Click
    ...
    RunService.RenderStepped:Connect(function()
        -- sync CollapseBtn ทุก frame (เบามาก แค่ set Position)
        SyncCollapseBtn()
