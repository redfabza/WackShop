...
function Library:NewWindow(config)
    ...
    -- ปุ่มเล็กๆ แปะขอบขวาของ Sidebar (child ของ Sidebar → ขยับตามอัตโนมัติ)
    local CollapseBtn = Instance.new("TextButton", Sidebar)
    CollapseBtn.Size     = UDim2.new(0,16,0,40)
    CollapseBtn.Position = UDim2.new(1,-8,0.5,-20)
    ...
    local ANIM_T = 0.28   -- วินาที (ยิ่งมากยิ่ง smooth)

    -- [FIX] รองรับทั้ง Mouse และ Touch — ใช้ InputEnded แทน MouseButton1Click
    -- ป้องกัน tween rotation ถูก hover cancel ด้วย TweenK key แยก
    local function CollapseToggle()
        ...
        if sideCollapsed then
            ...
            task.delay(ANIM_T*0.35, function()
                ...
                -- ใช้ key "_rot" แยก ไม่ถูก hover tween cancel
